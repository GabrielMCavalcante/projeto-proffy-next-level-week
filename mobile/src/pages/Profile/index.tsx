...
// Utils
import { formatFetchedPhone, formatCurrentPhone, formatToHoursAndMinutes } from 'utils/format'
    ...
    useEffect(() => {
        ...
        axios.get("/get-profile", {
            ...
            .then(response => {
                ...
                setScheduleItems([...userData.schedule.map(s => ({
                    week_day: weekdays.filter(w => w.value === String(s.week_day))[0],
                    from: formatToHoursAndMinutes(Number(s.from)),
                    to: formatToHoursAndMinutes(Number(s.to))
                }))])
    ...
    return (
        ...
        >
            <SafeAreaView style={{ flex: 1 }}>
                <PageHeader title="Meu perfil" returnTo="landing" />
                {
                    loadingData
                        ? <ActivityIndicator
                            size={50}
